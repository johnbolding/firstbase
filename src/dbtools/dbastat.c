...
/*  
 *  dbastat.c - read an Apache log file and generate a database for stats gathering
 */
...
#define URL_SIZE 200
#define AGENT_SIZE 200
#define REFERRER_SIZE 400
#define EXT_SIZE 10
#define IP_SIZE 40
...
/*
   66.249.71.78 - - [07/Nov/2008:00:02:10 -0700] "GET
   /articles/press/2005/phelps_hits_050818_sbs.htm HTTP/1.1"
   200 31382
   " xxx xxx xxx xxx "
   "Mozilla/5.0 (compatible; Googlebot/2.1; http://www. google.com/bot.html)"
   326 31830 www.horseweb.com
*/
 ...
 */
         ...
         if (p == 0)
            return(0);
         buf = p + 1;
         /* buf now points to one past quote */
         
         /* referrer field */
         fb_subline(referrer, buf, 1, '\"');
         /* agent field */
         fb_subline(agent, buf, 3, '\"');
         
         p = strrchr(buf, '\"');
         if (p == 0)
            return(0);
         buf = p + 2; /* eat quote and blank after */
         fb_subline(w3, buf, 1, FB_BLANK);
         fb_subline(w4, buf, 2, FB_BLANK);
         fb_subline(w5, buf, 3, FB_BLANK);
         ...
         if (!equal(code, "301") && !equal(code, "302")){
            p = strrchr(url, '.');
            if (p > 0){
               strcpy(ext, p+1);
               p = strrchr(ext, ';');
...
#if DEBUG
   printf("host=%s date=%s h=%s m=%s s=%s\nurl=%s\nagent=%s\n",
   host, date, hour, minute, second, url, agent);
   printf("referrer=%s\n", referrer);
   printf("bytes=%s ext=%s code=%s auth=%s ibytes=%s obytes=%s ip=%s\n",
   bytes, ext, code, auth, ibytes, obytes, ip);
#endif
...
return(FB_AOK);
         ...
         fb_store(db->kp[IBYTES], 	ibytes, db);
         fb_store(db->kp[OBYTES], 	obytes, db);
